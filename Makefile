...
ifeq ($(OS),Windows_NT)
	ALL_OBJECTS := $(ALL_OBJECTS) client_patcher.o client_launcher.o updater.o sha1.o
endif
...
build/client_patcher: $(OBJECTS:%.o=build/%.o) build/client_patcher.o
	...
	gcc $(CFLAGS) -o $@ $^ patcher_rc.o -s -Wl,--subsystem,windows -lwininet -mwindows -lcrypt32 -Wl,-Bstatic -liconv -Wl,-Bdynamic
